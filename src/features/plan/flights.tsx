...
export function flightPreviewText(f: FlightInfo | null): ReactNode {
  if (!f) return '';
  const airlineText = f.airline ? `${f.airline} · ` : '';
  const depText = f.dep.iata || f.dep.name || '?';
  const arrText = f.arr.iata || f.arr.name || '?';
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><CheckCircle size={14} /> {airlineText}{depText} {f.dep.time || ''} 출발 → {arrText} {f.arr.time || ''} 도착{f.manual ? ' (직접 입력)' : ''}</span>;
}
